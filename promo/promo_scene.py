...
class ProductPromo(Scene):
    def construct(self):
        ...
        # ═══ INTRO ═══
        title = Text("eDB", font_size=96, color=WHITE, weight=BOLD)
        underline = Line(LEFT * 3, RIGHT * 3, color=ACCENT, stroke_width=4)
        underline.next_to(title, DOWN, buff=0.3)
        tagline = Text("Unified Multi-Model Database", font_size=28, color=GRAY_B)
        tagline.next_to(underline, DOWN, buff=0.4)
        # Tech badges
        techs = "Python, SQLAlchemy, PostgreSQL".split(", ")
        badges = VGroup()
        for t in techs:
            badge = VGroup(
        ...
        # ═══ FEATURES ═══
        features = [
            ("01", "Relational + Document + Graph", "Single engine supports SQL tables, JSON documents, and graph traversals", DUR["f1"]),
            ("02", "Real-Time Replication", "Raft consensus protocol keeps replicas consistent with sub-millisecond lag", DUR["f2"]),
            ("03", "Embedded SQL Engine", "Runs inside your application process with zero network overhead", DUR["f3"]),
        ]
        ...
        components = ["Query Parser", "Optimizer", "Storage Engine", "Replication", "WAL"]
        boxes = VGroup()
